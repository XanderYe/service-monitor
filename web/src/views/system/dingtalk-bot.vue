...
<script>
  export default {
    ...
    data() {
      return {
        columns: [
          ...
          {title: "TOKEN", width: 200, name: "token"},
          {title: "SECRET", width: 200, name: "secret"},
    ...
    methods: {
      addPage() {
        this.$router.push({path: "/bot-edit"});
      },
      editPage(id) {
        this.$router.push({path: "/bot-edit", query: {id: id}});
      },
      getDataList() {
      ...
      deleteData() {
        ...
        this.$requests.post("/dingtalkBot/delete", formData).then(res => {
          if (res.data.code === 0) {
            this.$snackbar("删除成功");
